import cli1_img from "../images/items/1/1.jpg";
import cli2_img from "../images/items/2/1.jpg";
import cli3_img from "../images/items/3/1.jpg";
import cli4_img from "../images/items/4/1.jpg";
import cli5_img from "../images/items/5/1.jpg";
import cli6_img from "../images/items/6/1.jpg";

const items_data = [
    {
      id: 0,
      name: 'Madagascar Centella Asiatica Extract',
      mainImage: cli1_img,
      otherImages: [cli1_img, cli2_img, cli3_img],
      description: 'Whitening Cream For Sensitive Areas Moisturizer, Softener, And Soothing Leaves A Feeling Of Freshness Throughout The Day Use It As A Daily Moisturizer, Especially If You Suffer From Skin Rash, Itching, Burning Sensation Due To Sweating, White Secretions, Menstruation Period, Thigh Friction, Hair Removal. Suitable For Women Of All Ages And Skin Types',
      categoryId: 0,
      subCategoryId: 0,
      category: 'Hair Care',
      isNew: true,
      isPopular: true,
      popularTitle: 'Discover the Benefits of Dos Lunas’ Argan Conditioner for Lustrous and Nourished Hair',
      popularDescription: 'Dos Lunas is a cosmetic company that is dedicated to producing high-quality beauty products that…',
      createdDate: '2024-01-15',
      isOnSale: true,
      isOnQuickAccess: true
    }
    ,
    {
      id: 1,
      name: 'Speedy Hair Color Shampoo Burgundy Red',
      mainImage: cli2_img,
      otherImages:[cli1_img, cli2_img, cli3_img],
      description: 'Whitening Cream For Sensitive Areas Moisturizer, Softener, And Soothing Leaves A Feeling Of Freshness Throughout The Day Use It As A Daily Moisturizer, Especially If You Suffer From Skin Rash, Itching, Burning Sensation Due To Sweating, White Secretions, Menstruation Period, Thigh Friction, Hair Removal. Suitable For Women Of All Ages And Skin Types',
      categoryId: 0,
      subCategoryId: 0,
      category: 'Hair Care',
      isNew: true,
      isPopular: true,
      popularTitle: 'Experience Nourishing & Perfumed Hair with Dos Lunas Hair Spray Collection',
      popularDescription: 'Introducing the Nourishing & Perfumed Hair Spray Collection from Dos Lunas! This collection features two...',
      createdDate: '2024-01-15',
      isOnSale: true,
      isOnQuickAccess: true
    }
    ,
    {
      id: 2,
      name: 'Hair Oil Avocado',
      mainImage: cli3_img,
      otherImages:[cli1_img, cli2_img, cli3_img],
      description: 'Whitening Cream For Sensitive Areas Moisturizer, Softener, And Soothing Leaves A Feeling Of Freshness Throughout The Day Use It As A Daily Moisturizer, Especially If You Suffer From Skin Rash, Itching, Burning Sensation Due To Sweating, White Secretions, Menstruation Period, Thigh Friction, Hair Removal. Suitable For Women Of All Ages And Skin Types',
      categoryId: 0,
      subCategoryId: 0,
      category: 'Hair Care',
      isNew: true,
      isPopular: true,
      popularTitle: 'Revitalize Your Hair with Dos Lunas\' Hair Mas Moisturizer',
      popularDescription: 'Introduction the Hair Mask Moisturizer from Dos Lunas! This hair mask is the perfect solution...',
      createdDate: '2024-01-15',
      isOnSale: true,
      isOnQuickAccess: true
    }
    ,
    {
      id: 3,
      name: 'Hair Oil Coconut',
      mainImage: cli4_img,
      otherImages:[cli1_img, cli2_img, cli3_img],
      description: 'Whitening Cream For Sensitive Areas Moisturizer, Softener, And Soothing Leaves A Feeling Of Freshness Throughout The Day Use It As A Daily Moisturizer, Especially If You Suffer From Skin Rash, Itching, Burning Sensation Due To Sweating, White Secretions, Menstruation Period, Thigh Friction, Hair Removal. Suitable For Women Of All Ages And Skin Types',
      categoryId: 0,
      subCategoryId: 1,
      category: 'Hair Care',
      isNew: false,
      isPopular: false,
      popularTitle: '',
      popularDescription: '',
      createdDate: '2024-01-15',
      isOnSale: false,
      isOnQuickAccess: true
    }
    ,
    {
      id: 4,
      name: 'Hair Oil Argan',
      mainImage: cli5_img,
      otherImages:[cli1_img, cli2_img, cli3_img],
      description: 'Whitening Cream For Sensitive Areas Moisturizer, Softener, And Soothing Leaves A Feeling Of Freshness Throughout The Day Use It As A Daily Moisturizer, Especially If You Suffer From Skin Rash, Itching, Burning Sensation Due To Sweating, White Secretions, Menstruation Period, Thigh Friction, Hair Removal. Suitable For Women Of All Ages And Skin Types',
      categoryId: 0,
      subCategoryId: 1,
      category: 'Hair Care',
      isNew: false,
      isPopular: false,
      popularTitle: '',
      popularDescription: '',
      createdDate: '2024-01-15',
      isOnSale: false,
      isOnQuickAccess: false
    }
    ,
    {
      id: 5,
      name: 'Hair Oil Rosemary',
      mainImage: cli6_img,
      otherImages:[cli1_img, cli2_img, cli3_img],
      description: 'Whitening Cream For Sensitive Areas Moisturizer, Softener, And Soothing Leaves A Feeling Of Freshness Throughout The Day Use It As A Daily Moisturizer, Especially If You Suffer From Skin Rash, Itching, Burning Sensation Due To Sweating, White Secretions, Menstruation Period, Thigh Friction, Hair Removal. Suitable For Women Of All Ages And Skin Types',
      categoryId: 0,
      subCategoryId: 1,
      category: 'Hair Care',
      isNew: false,
      isPopular: false,
      popularTitle: '',
      popularDescription: '',
      createdDate: '2024-01-15',
      isOnSale: false,
      isOnQuickAccess: false
    },
    {
      id: 6,
      name: 'Hair Oil Rosemary',
      mainImage: cli1_img,
      otherImages:[cli1_img, cli2_img, cli3_img],
      description: 'Whitening Cream For Sensitive Areas Moisturizer, Softener, And Soothing Leaves A Feeling Of Freshness Throughout The Day Use It As A Daily Moisturizer, Especially If You Suffer From Skin Rash, Itching, Burning Sensation Due To Sweating, White Secretions, Menstruation Period, Thigh Friction, Hair Removal. Suitable For Women Of All Ages And Skin Types',
      categoryId: 0,
      subCategoryId: 1,
      category: 'Hair Care',
      isNew: false,
      isPopular: false,
      popularTitle: '',
      popularDescription: '',
      createdDate: '2024-01-15',
      isOnSale: false,
      isOnQuickAccess: false
    },
    {
      id: 7,
      name: 'Hair Oil Rosemary',
      mainImage: cli1_img,
      otherImages:[cli1_img, cli2_img, cli3_img],
      description: 'Whitening Cream For Sensitive Areas Moisturizer, Softener, And Soothing Leaves A Feeling Of Freshness Throughout The Day Use It As A Daily Moisturizer, Especially If You Suffer From Skin Rash, Itching, Burning Sensation Due To Sweating, White Secretions, Menstruation Period, Thigh Friction, Hair Removal. Suitable For Women Of All Ages And Skin Types',
      categoryId: 0,
      subCategoryId: 1,
      category: 'Hair Care',
      isNew: true,
      isPopular: false,
      popularTitle: '',
      popularDescription: '',
      createdDate: '2024-01-15',
      isOnSale: false,
      isOnQuickAccess: false
    },
    {
      id: 8,
      name: 'Hair Oil Rosemary',
      mainImage: cli1_img,
      otherImages:[cli1_img, cli2_img, cli3_img],
      description: 'Whitening Cream For Sensitive Areas Moisturizer, Softener, And Soothing Leaves A Feeling Of Freshness Throughout The Day Use It As A Daily Moisturizer, Especially If You Suffer From Skin Rash, Itching, Burning Sensation Due To Sweating, White Secretions, Menstruation Period, Thigh Friction, Hair Removal. Suitable For Women Of All Ages And Skin Types',
      categoryId: 0,
      subCategoryId: 1,
      category: 'Hair Care',
      isNew: true,
      isPopular: false,
      popularTitle: '',
      popularDescription: '',
      createdDate: '2024-01-15',
      isOnSale: false,
      isOnQuickAccess: false
    },
    {
      id: 9,
      name: 'Hair Oil Rosemary',
      mainImage: cli1_img,
      otherImages:[cli1_img, cli2_img, cli3_img],
      description: 'Whitening Cream For Sensitive Areas Moisturizer, Softener, And Soothing Leaves A Feeling Of Freshness Throughout The Day Use It As A Daily Moisturizer, Especially If You Suffer From Skin Rash, Itching, Burning Sensation Due To Sweating, White Secretions, Menstruation Period, Thigh Friction, Hair Removal. Suitable For Women Of All Ages And Skin Types',
      categoryId: 0,
      subCategoryId: 1,
      category: 'Hair Care',
      isNew: true,
      isPopular: false,
      popularTitle: '',
      popularDescription: '',
      createdDate: '2024-01-15',
      isOnSale: false,
      isOnQuickAccess: false
    },
    {
      id: 10,
      name: 'Hair Oil Rosemary',
      mainImage: cli1_img,
      otherImages:[cli1_img, cli2_img, cli3_img],
      description: 'Whitening Cream For Sensitive Areas Moisturizer, Softener, And Soothing Leaves A Feeling Of Freshness Throughout The Day Use It As A Daily Moisturizer, Especially If You Suffer From Skin Rash, Itching, Burning Sensation Due To Sweating, White Secretions, Menstruation Period, Thigh Friction, Hair Removal. Suitable For Women Of All Ages And Skin Types',
      categoryId: 0,
      subCategoryId: 1,
      category: 'Hair Care',
      isNew: true,
      isPopular: false,
      popularTitle: '',
      popularDescription: '',
      createdDate: '2024-01-15',
      isOnSale: false,
      isOnQuickAccess: false
    },
    {
      id: 11,
      name: 'Hair Oil Rosemary',
      mainImage: cli1_img,
      otherImages:[cli1_img, cli2_img, cli3_img],
      description: 'Whitening Cream For Sensitive Areas Moisturizer, Softener, And Soothing Leaves A Feeling Of Freshness Throughout The Day Use It As A Daily Moisturizer, Especially If You Suffer From Skin Rash, Itching, Burning Sensation Due To Sweating, White Secretions, Menstruation Period, Thigh Friction, Hair Removal. Suitable For Women Of All Ages And Skin Types',
      categoryId: 0,
      subCategoryId: 1,
      category: 'Hair Care',
      isNew: true,
      isPopular: false,
      popularTitle: '',
      popularDescription: '',
      createdDate: '2024-01-15',
      isOnSale: false,
      isOnQuickAccess: false
    },
    {
      id: 12,
      name: 'Hair Oil Rosemary',
      mainImage: cli1_img,
      otherImages:[cli1_img, cli2_img, cli3_img],
      description: 'Whitening Cream For Sensitive Areas Moisturizer, Softener, And Soothing Leaves A Feeling Of Freshness Throughout The Day Use It As A Daily Moisturizer, Especially If You Suffer From Skin Rash, Itching, Burning Sensation Due To Sweating, White Secretions, Menstruation Period, Thigh Friction, Hair Removal. Suitable For Women Of All Ages And Skin Types',
      categoryId: 0,
      subCategoryId: 1,
      category: 'Hair Care',
      isNew: true,
      isPopular: false,
      popularTitle: '',
      popularDescription: '',
      createdDate: '2024-01-15',
      isOnSale: false,
      isOnQuickAccess: false
    },
    {
      id: 13,
      name: 'Hair Oil Rosemary',
      mainImage: cli1_img,
      otherImages:[cli1_img, cli2_img, cli3_img],
      description: 'Whitening Cream For Sensitive Areas Moisturizer, Softener, And Soothing Leaves A Feeling Of Freshness Throughout The Day Use It As A Daily Moisturizer, Especially If You Suffer From Skin Rash, Itching, Burning Sensation Due To Sweating, White Secretions, Menstruation Period, Thigh Friction, Hair Removal. Suitable For Women Of All Ages And Skin Types',
      categoryId: 0,
      subCategoryId: 1,
      category: 'Hair Care',
      isNew: true,
      isPopular: false,
      popularTitle: '',
      popularDescription: '',
      createdDate: '2024-01-15',
      isOnSale: false,
      isOnQuickAccess: false
    },
    {
      id: 14,
      name: 'Hair Oil Rosemary',
      mainImage: cli1_img,
      otherImages:[cli1_img, cli2_img, cli3_img],
      description: 'Whitening Cream For Sensitive Areas Moisturizer, Softener, And Soothing Leaves A Feeling Of Freshness Throughout The Day Use It As A Daily Moisturizer, Especially If You Suffer From Skin Rash, Itching, Burning Sensation Due To Sweating, White Secretions, Menstruation Period, Thigh Friction, Hair Removal. Suitable For Women Of All Ages And Skin Types',
      categoryId: 0,
      subCategoryId: 1,
      category: 'Hair Care',
      isNew: true,
      isPopular: false,
      popularTitle: '',
      popularDescription: '',
      createdDate: '2024-01-15',
      isOnSale: false,
      isOnQuickAccess: false
    },
    {
      id: 15,
      name: 'Hair Oil Rosemary',
      mainImage: cli1_img,
      otherImages:[cli1_img, cli2_img, cli3_img],
      description: 'Whitening Cream For Sensitive Areas Moisturizer, Softener, And Soothing Leaves A Feeling Of Freshness Throughout The Day Use It As A Daily Moisturizer, Especially If You Suffer From Skin Rash, Itching, Burning Sensation Due To Sweating, White Secretions, Menstruation Period, Thigh Friction, Hair Removal. Suitable For Women Of All Ages And Skin Types',
      categoryId: 0,
      subCategoryId: 1,
      category: 'Hair Care',
      isNew: true,
      isPopular: false,
      popularTitle: '',
      popularDescription: '',
      createdDate: '2024-01-15',
      isOnSale: false,
      isOnQuickAccess: false
    },
    {
      id: 16,
      name: 'Hair Oil Rosemary',
      mainImage: cli1_img,
      otherImages:[cli1_img, cli2_img, cli3_img],
      description: 'Whitening Cream For Sensitive Areas Moisturizer, Softener, And Soothing Leaves A Feeling Of Freshness Throughout The Day Use It As A Daily Moisturizer, Especially If You Suffer From Skin Rash, Itching, Burning Sensation Due To Sweating, White Secretions, Menstruation Period, Thigh Friction, Hair Removal. Suitable For Women Of All Ages And Skin Types',
      categoryId: 0,
      subCategoryId: 1,
      category: 'Hair Care',
      isNew: true,
      isPopular: false,
      popularTitle: '',
      popularDescription: '',
      createdDate: '2024-01-15',
      isOnSale: false,
      isOnQuickAccess: false
    },
    {
      id: 17,
      name: 'Hair Oil Rosemary',
      mainImage: cli1_img,
      otherImages:[cli1_img, cli2_img, cli3_img],
      description: 'Whitening Cream For Sensitive Areas Moisturizer, Softener, And Soothing Leaves A Feeling Of Freshness Throughout The Day Use It As A Daily Moisturizer, Especially If You Suffer From Skin Rash, Itching, Burning Sensation Due To Sweating, White Secretions, Menstruation Period, Thigh Friction, Hair Removal. Suitable For Women Of All Ages And Skin Types',
      categoryId: 0,
      subCategoryId: 1,
      category: 'Hair Care',
      isNew: true,
      isPopular: false,
      popularTitle: '',
      popularDescription: '',
      createdDate: '2024-01-15',
      isOnSale: false,
      isOnQuickAccess: false
    },
    {
      id: 18,
      name: 'Hair Oil Rosemary',
      mainImage: cli1_img,
      otherImages:[cli1_img, cli2_img, cli3_img],
      description: 'Whitening Cream For Sensitive Areas Moisturizer, Softener, And Soothing Leaves A Feeling Of Freshness Throughout The Day Use It As A Daily Moisturizer, Especially If You Suffer From Skin Rash, Itching, Burning Sensation Due To Sweating, White Secretions, Menstruation Period, Thigh Friction, Hair Removal. Suitable For Women Of All Ages And Skin Types',
      categoryId: 0,
      subCategoryId: 1,
      category: 'Hair Care',
      isNew: true,
      isPopular: false,
      popularTitle: '',
      popularDescription: '',
      createdDate: '2024-01-15',
      isOnSale: false,
      isOnQuickAccess: false
    },
    {
      id: 19,
      name: 'Hair Oil Rosemary',
      mainImage: cli1_img,
      otherImages:[cli1_img, cli2_img, cli3_img],
      description: 'Whitening Cream For Sensitive Areas Moisturizer, Softener, And Soothing Leaves A Feeling Of Freshness Throughout The Day Use It As A Daily Moisturizer, Especially If You Suffer From Skin Rash, Itching, Burning Sensation Due To Sweating, White Secretions, Menstruation Period, Thigh Friction, Hair Removal. Suitable For Women Of All Ages And Skin Types',
      categoryId: 0,
      subCategoryId: 1,
      category: 'Hair Care',
      isNew: true,
      isPopular: false,
      popularTitle: '',
      popularDescription: '',
      createdDate: '2024-01-15',
      isOnSale: false,
      isOnQuickAccess: false
    },
    {
      id: 20,
      name: 'Hair Oil Rosemary',
      mainImage: cli1_img,
      otherImages:[cli1_img, cli2_img, cli3_img],
      description: 'Whitening Cream For Sensitive Areas Moisturizer, Softener, And Soothing Leaves A Feeling Of Freshness Throughout The Day Use It As A Daily Moisturizer, Especially If You Suffer From Skin Rash, Itching, Burning Sensation Due To Sweating, White Secretions, Menstruation Period, Thigh Friction, Hair Removal. Suitable For Women Of All Ages And Skin Types',
      categoryId: 0,
      subCategoryId: 1,
      category: 'Hair Care',
      isNew: true,
      isPopular: false,
      popularTitle: '',
      popularDescription: '',
      createdDate: '2024-01-15',
      isOnSale: false,
      isOnQuickAccess: false
    },
    {
      id: 21,
      name: 'Hair Oil Rosemary',
      mainImage: cli1_img,
      otherImages:[cli1_img, cli2_img, cli3_img],
      description: 'Whitening Cream For Sensitive Areas Moisturizer, Softener, And Soothing Leaves A Feeling Of Freshness Throughout The Day Use It As A Daily Moisturizer, Especially If You Suffer From Skin Rash, Itching, Burning Sensation Due To Sweating, White Secretions, Menstruation Period, Thigh Friction, Hair Removal. Suitable For Women Of All Ages And Skin Types',
      categoryId: 0,
      subCategoryId: 1,
      category: 'Hair Care',
      isNew: true,
      isPopular: false,
      popularTitle: '',
      popularDescription: '',
      createdDate: '2024-01-15',
      isOnSale: false,
      isOnQuickAccess: false
    },
    {
      id: 22,
      name: 'Hair Oil Rosemary',
      mainImage: cli1_img,
      otherImages:[cli1_img, cli2_img, cli3_img],
      description: 'Whitening Cream For Sensitive Areas Moisturizer, Softener, And Soothing Leaves A Feeling Of Freshness Throughout The Day Use It As A Daily Moisturizer, Especially If You Suffer From Skin Rash, Itching, Burning Sensation Due To Sweating, White Secretions, Menstruation Period, Thigh Friction, Hair Removal. Suitable For Women Of All Ages And Skin Types',
      categoryId: 0,
      subCategoryId: 1,
      category: 'Hair Care',
      isNew: true,
      isPopular: false,
      popularTitle: '',
      popularDescription: '',
      createdDate: '2024-01-15',
      isOnSale: false,
      isOnQuickAccess: false
    },
    {
      id: 23,
      name: 'Hair Oil Rosemary',
      mainImage: cli1_img,
      otherImages:[cli1_img, cli2_img, cli3_img],
      description: 'Whitening Cream For Sensitive Areas Moisturizer, Softener, And Soothing Leaves A Feeling Of Freshness Throughout The Day Use It As A Daily Moisturizer, Especially If You Suffer From Skin Rash, Itching, Burning Sensation Due To Sweating, White Secretions, Menstruation Period, Thigh Friction, Hair Removal. Suitable For Women Of All Ages And Skin Types',
      categoryId: 0,
      subCategoryId: 1,
      category: 'Hair Care',
      isNew: true,
      isPopular: false,
      popularTitle: '',
      popularDescription: '',
      createdDate: '2024-01-15',
      isOnSale: false,
      isOnQuickAccess: false
    },
    {
      id: 24,
      name: 'Hair Oil Rosemary',
      mainImage: cli1_img,
      otherImages:[cli1_img, cli2_img, cli3_img],
      description: 'Whitening Cream For Sensitive Areas Moisturizer, Softener, And Soothing Leaves A Feeling Of Freshness Throughout The Day Use It As A Daily Moisturizer, Especially If You Suffer From Skin Rash, Itching, Burning Sensation Due To Sweating, White Secretions, Menstruation Period, Thigh Friction, Hair Removal. Suitable For Women Of All Ages And Skin Types',
      categoryId: 0,
      subCategoryId: 1,
      category: 'Hair Care',
      isNew: true,
      isPopular: false,
      popularTitle: '',
      popularDescription: '',
      createdDate: '2024-01-15',
      isOnSale: false,
      isOnQuickAccess: false
    },
    {
      id: 25,
      name: 'Hair Oil Rosemary',
      mainImage: cli1_img,
      otherImages:[cli1_img, cli2_img, cli3_img],
      description: 'Whitening Cream For Sensitive Areas Moisturizer, Softener, And Soothing Leaves A Feeling Of Freshness Throughout The Day Use It As A Daily Moisturizer, Especially If You Suffer From Skin Rash, Itching, Burning Sensation Due To Sweating, White Secretions, Menstruation Period, Thigh Friction, Hair Removal. Suitable For Women Of All Ages And Skin Types',
      categoryId: 0,
      subCategoryId: 1,
      category: 'Hair Care',
      isNew: true,
      isPopular: false,
      popularTitle: '',
      popularDescription: '',
      createdDate: '2024-01-15',
      isOnSale: false,
      isOnQuickAccess: false
    },
    {
      id: 26,
      name: 'Hair Oil Rosemary',
      mainImage: cli1_img,
      otherImages:[cli1_img, cli2_img, cli3_img],
      description: 'Whitening Cream For Sensitive Areas Moisturizer, Softener, And Soothing Leaves A Feeling Of Freshness Throughout The Day Use It As A Daily Moisturizer, Especially If You Suffer From Skin Rash, Itching, Burning Sensation Due To Sweating, White Secretions, Menstruation Period, Thigh Friction, Hair Removal. Suitable For Women Of All Ages And Skin Types',
      categoryId: 0,
      subCategoryId: 1,
      category: 'Hair Care',
      isNew: true,
      isPopular: false,
      popularTitle: '',
      popularDescription: '',
      createdDate: '2024-01-15',
      isOnSale: false,
      isOnQuickAccess: false
    },
    {
      id: 27,
      name: 'Hair Oil Rosemary',
      mainImage: cli1_img,
      otherImages:[cli1_img, cli2_img, cli3_img],
      description: 'Whitening Cream For Sensitive Areas Moisturizer, Softener, And Soothing Leaves A Feeling Of Freshness Throughout The Day Use It As A Daily Moisturizer, Especially If You Suffer From Skin Rash, Itching, Burning Sensation Due To Sweating, White Secretions, Menstruation Period, Thigh Friction, Hair Removal. Suitable For Women Of All Ages And Skin Types',
      categoryId: 0,
      subCategoryId: 1,
      category: 'Hair Care',
      isNew: true,
      isPopular: false,
      popularTitle: '',
      popularDescription: '',
      createdDate: '2024-01-15',
      isOnSale: false,
      isOnQuickAccess: false
    },
    {
      id: 28,
      name: 'Hair Oil Rosemary',
      mainImage: cli1_img,
      otherImages:[cli1_img, cli2_img, cli3_img],
      description: 'Whitening Cream For Sensitive Areas Moisturizer, Softener, And Soothing Leaves A Feeling Of Freshness Throughout The Day Use It As A Daily Moisturizer, Especially If You Suffer From Skin Rash, Itching, Burning Sensation Due To Sweating, White Secretions, Menstruation Period, Thigh Friction, Hair Removal. Suitable For Women Of All Ages And Skin Types',
      categoryId: 0,
      subCategoryId: 1,
      category: 'Hair Care',
      isNew: true,
      isPopular: false,
      popularTitle: '',
      popularDescription: '',
      createdDate: '2024-01-15',
      isOnSale: false,
      isOnQuickAccess: false
    },
    {
      id: 29,
      name: 'Hair Oil Rosemary',
      mainImage: cli1_img,
      otherImages:[cli1_img, cli2_img, cli3_img],
      description: 'Whitening Cream For Sensitive Areas Moisturizer, Softener, And Soothing Leaves A Feeling Of Freshness Throughout The Day Use It As A Daily Moisturizer, Especially If You Suffer From Skin Rash, Itching, Burning Sensation Due To Sweating, White Secretions, Menstruation Period, Thigh Friction, Hair Removal. Suitable For Women Of All Ages And Skin Types',
      categoryId: 0,
      subCategoryId: 1,
      category: 'Hair Care',
      isNew: true,
      isPopular: false,
      popularTitle: '',
      popularDescription: '',
      createdDate: '2024-01-15',
      isOnSale: false,
      isOnQuickAccess: false
    },
    {
      id: 30,
      name: 'Hair Oil Rosemary',
      mainImage: cli1_img,
      otherImages:[cli1_img, cli2_img, cli3_img],
      description: 'Whitening Cream For Sensitive Areas Moisturizer, Softener, And Soothing Leaves A Feeling Of Freshness Throughout The Day Use It As A Daily Moisturizer, Especially If You Suffer From Skin Rash, Itching, Burning Sensation Due To Sweating, White Secretions, Menstruation Period, Thigh Friction, Hair Removal. Suitable For Women Of All Ages And Skin Types',
      categoryId: 0,
      subCategoryId: 1,
      category: 'Hair Care',
      isNew: true,
      isPopular: false,
      popularTitle: '',
      popularDescription: '',
      createdDate: '2024-01-15',
      isOnSale: false,
      isOnQuickAccess: false
    },
    {
      id: 31,
      name: 'Hair Oil Rosemary',
      mainImage: cli1_img,
      otherImages:[cli1_img, cli2_img, cli3_img],
      description: 'Whitening Cream For Sensitive Areas Moisturizer, Softener, And Soothing Leaves A Feeling Of Freshness Throughout The Day Use It As A Daily Moisturizer, Especially If You Suffer From Skin Rash, Itching, Burning Sensation Due To Sweating, White Secretions, Menstruation Period, Thigh Friction, Hair Removal. Suitable For Women Of All Ages And Skin Types',
      categoryId: 0,
      subCategoryId: 1,
      category: 'Hair Care',
      isNew: true,
      isPopular: false,
      popularTitle: '',
      popularDescription: '',
      createdDate: '2024-01-15',
      isOnSale: false,
      isOnQuickAccess: false
    },
    {
      id: 32,
      name: 'Hair Oil Rosemary',
      mainImage: cli1_img,
      otherImages:[cli1_img, cli2_img, cli3_img],
      description: 'Whitening Cream For Sensitive Areas Moisturizer, Softener, And Soothing Leaves A Feeling Of Freshness Throughout The Day Use It As A Daily Moisturizer, Especially If You Suffer From Skin Rash, Itching, Burning Sensation Due To Sweating, White Secretions, Menstruation Period, Thigh Friction, Hair Removal. Suitable For Women Of All Ages And Skin Types',
      categoryId: 0,
      subCategoryId: 1,
      category: 'Hair Care',
      isNew: true,
      isPopular: false,
      popularTitle: '',
      popularDescription: '',
      createdDate: '2024-01-15',
      isOnSale: false,
      isOnQuickAccess: false
    },
    {
      id: 33,
      name: 'Hair Oil Rosemary',
      mainImage: cli1_img,
      otherImages:[cli1_img, cli2_img, cli3_img],
      description: 'Whitening Cream For Sensitive Areas Moisturizer, Softener, And Soothing Leaves A Feeling Of Freshness Throughout The Day Use It As A Daily Moisturizer, Especially If You Suffer From Skin Rash, Itching, Burning Sensation Due To Sweating, White Secretions, Menstruation Period, Thigh Friction, Hair Removal. Suitable For Women Of All Ages And Skin Types',
      categoryId: 0,
      subCategoryId: 1,
      category: 'Hair Care',
      isNew: true,
      isPopular: false,
      popularTitle: '',
      popularDescription: '',
      createdDate: '2024-01-15',
      isOnSale: false,
      isOnQuickAccess: false
    },
    {
      id: 34,
      name: 'Hair Oil Rosemary',
      mainImage: cli1_img,
      otherImages:[cli1_img, cli2_img, cli3_img],
      description: 'Whitening Cream For Sensitive Areas Moisturizer, Softener, And Soothing Leaves A Feeling Of Freshness Throughout The Day Use It As A Daily Moisturizer, Especially If You Suffer From Skin Rash, Itching, Burning Sensation Due To Sweating, White Secretions, Menstruation Period, Thigh Friction, Hair Removal. Suitable For Women Of All Ages And Skin Types',
      categoryId: 0,
      subCategoryId: 1,
      category: 'Hair Care',
      isNew: true,
      isPopular: false,
      popularTitle: '',
      popularDescription: '',
      createdDate: '2024-01-15',
      isOnSale: false,
      isOnQuickAccess: false
    },
    {
      id: 35,
      name: 'Hair Oil Rosemary',
      mainImage: cli1_img,
      otherImages:[cli1_img, cli2_img, cli3_img],
      description: 'Whitening Cream For Sensitive Areas Moisturizer, Softener, And Soothing Leaves A Feeling Of Freshness Throughout The Day Use It As A Daily Moisturizer, Especially If You Suffer From Skin Rash, Itching, Burning Sensation Due To Sweating, White Secretions, Menstruation Period, Thigh Friction, Hair Removal. Suitable For Women Of All Ages And Skin Types',
      categoryId: 0,
      subCategoryId: 1,
      category: 'Hair Care',
      isNew: true,
      isPopular: false,
      popularTitle: '',
      popularDescription: '',
      createdDate: '2024-01-15',
      isOnSale: false,
      isOnQuickAccess: false
    },
    {
      id: 36,
      name: 'Hair Oil Rosemary',
      mainImage: cli1_img,
      otherImages:[cli1_img, cli2_img, cli3_img],
      description: 'Whitening Cream For Sensitive Areas Moisturizer, Softener, And Soothing Leaves A Feeling Of Freshness Throughout The Day Use It As A Daily Moisturizer, Especially If You Suffer From Skin Rash, Itching, Burning Sensation Due To Sweating, White Secretions, Menstruation Period, Thigh Friction, Hair Removal. Suitable For Women Of All Ages And Skin Types',
      categoryId: 0,
      subCategoryId: 1,
      category: 'Hair Care',
      isNew: true,
      isPopular: false,
      popularTitle: '',
      popularDescription: '',
      createdDate: '2024-01-15',
      isOnSale: false,
      isOnQuickAccess: false
    },
    {
      id: 37,
      name: 'Hair Oil Rosemary',
      mainImage: cli1_img,
      otherImages:[cli1_img, cli2_img, cli3_img],
      description: 'Whitening Cream For Sensitive Areas Moisturizer, Softener, And Soothing Leaves A Feeling Of Freshness Throughout The Day Use It As A Daily Moisturizer, Especially If You Suffer From Skin Rash, Itching, Burning Sensation Due To Sweating, White Secretions, Menstruation Period, Thigh Friction, Hair Removal. Suitable For Women Of All Ages And Skin Types',
      categoryId: 0,
      subCategoryId: 1,
      category: 'Hair Care',
      isNew: true,
      isPopular: false,
      popularTitle: '',
      popularDescription: '',
      createdDate: '2024-01-15',
      isOnSale: false,
      isOnQuickAccess: false
    },
    {
      id: 38,
      name: 'Hair Oil Rosemary',
      mainImage: cli1_img,
      otherImages:[cli1_img, cli2_img, cli3_img],
      description: 'Whitening Cream For Sensitive Areas Moisturizer, Softener, And Soothing Leaves A Feeling Of Freshness Throughout The Day Use It As A Daily Moisturizer, Especially If You Suffer From Skin Rash, Itching, Burning Sensation Due To Sweating, White Secretions, Menstruation Period, Thigh Friction, Hair Removal. Suitable For Women Of All Ages And Skin Types',
      categoryId: 0,
      subCategoryId: 1,
      category: 'Hair Care',
      isNew: true,
      isPopular: false,
      popularTitle: '',
      popularDescription: '',
      createdDate: '2024-01-15',
      isOnSale: false,
      isOnQuickAccess: false
    },
    {
      id: 39,
      name: 'Hair Oil Rosemary',
      mainImage: cli1_img,
      otherImages:[cli1_img, cli2_img, cli3_img],
      description: 'Whitening Cream For Sensitive Areas Moisturizer, Softener, And Soothing Leaves A Feeling Of Freshness Throughout The Day Use It As A Daily Moisturizer, Especially If You Suffer From Skin Rash, Itching, Burning Sensation Due To Sweating, White Secretions, Menstruation Period, Thigh Friction, Hair Removal. Suitable For Women Of All Ages And Skin Types',
      categoryId: 0,
      subCategoryId: 1,
      category: 'Hair Care',
      isNew: true,
      isPopular: false,
      popularTitle: '',
      popularDescription: '',
      createdDate: '2024-01-15',
      isOnSale: false,
      isOnQuickAccess: false
    },
    {
      id: 40,
      name: 'Hair Oil Rosemary',
      mainImage: cli1_img,
      otherImages:[cli1_img, cli2_img, cli3_img],
      description: 'Whitening Cream For Sensitive Areas Moisturizer, Softener, And Soothing Leaves A Feeling Of Freshness Throughout The Day Use It As A Daily Moisturizer, Especially If You Suffer From Skin Rash, Itching, Burning Sensation Due To Sweating, White Secretions, Menstruation Period, Thigh Friction, Hair Removal. Suitable For Women Of All Ages And Skin Types',
      categoryId: 0,
      subCategoryId: 1,
      category: 'Hair Care',
      isNew: true,
      isPopular: false,
      popularTitle: '',
      popularDescription: '',
      createdDate: '2024-01-15',
      isOnSale: false,
      isOnQuickAccess: false
    },
    {
      id: 41,
      name: 'Hair Oil Rosemary',
      mainImage: cli1_img,
      otherImages:[cli1_img, cli2_img, cli3_img],
      description: 'Whitening Cream For Sensitive Areas Moisturizer, Softener, And Soothing Leaves A Feeling Of Freshness Throughout The Day Use It As A Daily Moisturizer, Especially If You Suffer From Skin Rash, Itching, Burning Sensation Due To Sweating, White Secretions, Menstruation Period, Thigh Friction, Hair Removal. Suitable For Women Of All Ages And Skin Types',
      categoryId: 0,
      subCategoryId: 1,
      category: 'Hair Care',
      isNew: true,
      isPopular: false,
      popularTitle: '',
      popularDescription: '',
      createdDate: '2024-01-15',
      isOnSale: false,
      isOnQuickAccess: false
    },
    {
      id: 42,
      name: 'Hair Oil Rosemary',
      mainImage: cli1_img,
      otherImages:[cli1_img, cli2_img, cli3_img],
      description: 'Whitening Cream For Sensitive Areas Moisturizer, Softener, And Soothing Leaves A Feeling Of Freshness Throughout The Day Use It As A Daily Moisturizer, Especially If You Suffer From Skin Rash, Itching, Burning Sensation Due To Sweating, White Secretions, Menstruation Period, Thigh Friction, Hair Removal. Suitable For Women Of All Ages And Skin Types',
      categoryId: 0,
      subCategoryId: 1,
      category: 'Hair Care',
      isNew: true,
      isPopular: false,
      popularTitle: '',
      popularDescription: '',
      createdDate: '2024-01-15',
      isOnSale: false,
      isOnQuickAccess: false
    },
    {
      id: 43,
      name: 'Hair Oil Rosemary',
      mainImage: cli1_img,
      otherImages:[cli1_img, cli2_img, cli3_img],
      description: 'Whitening Cream For Sensitive Areas Moisturizer, Softener, And Soothing Leaves A Feeling Of Freshness Throughout The Day Use It As A Daily Moisturizer, Especially If You Suffer From Skin Rash, Itching, Burning Sensation Due To Sweating, White Secretions, Menstruation Period, Thigh Friction, Hair Removal. Suitable For Women Of All Ages And Skin Types',
      categoryId: 0,
      subCategoryId: 1,
      category: 'Hair Care',
      isNew: true,
      isPopular: false,
      popularTitle: '',
      popularDescription: '',
      createdDate: '2024-01-15',
      isOnSale: false,
      isOnQuickAccess: false
    },
    {
      id: 44,
      name: 'Hair Oil Rosemary',
      mainImage: cli1_img,
      otherImages:[cli1_img, cli2_img, cli3_img],
      description: 'Whitening Cream For Sensitive Areas Moisturizer, Softener, And Soothing Leaves A Feeling Of Freshness Throughout The Day Use It As A Daily Moisturizer, Especially If You Suffer From Skin Rash, Itching, Burning Sensation Due To Sweating, White Secretions, Menstruation Period, Thigh Friction, Hair Removal. Suitable For Women Of All Ages And Skin Types',
      categoryId: 0,
      subCategoryId: 1,
      category: 'Hair Care',
      isNew: true,
      isPopular: false,
      popularTitle: '',
      popularDescription: '',
      createdDate: '2024-01-15',
      isOnSale: false,
      isOnQuickAccess: false
    },
    {
      id: 45,
      name: 'Hair Oil Rosemary',
      mainImage: cli1_img,
      otherImages:[cli1_img, cli2_img, cli3_img],
      description: 'Whitening Cream For Sensitive Areas Moisturizer, Softener, And Soothing Leaves A Feeling Of Freshness Throughout The Day Use It As A Daily Moisturizer, Especially If You Suffer From Skin Rash, Itching, Burning Sensation Due To Sweating, White Secretions, Menstruation Period, Thigh Friction, Hair Removal. Suitable For Women Of All Ages And Skin Types',
      categoryId: 0,
      subCategoryId: 1,
      category: 'Hair Care',
      isNew: true,
      isPopular: false,
      popularTitle: '',
      popularDescription: '',
      createdDate: '2024-01-15',
      isOnSale: false,
      isOnQuickAccess: false
    },
    {
      id: 46,
      name: 'Hair Oil Rosemary',
      mainImage: cli1_img,
      otherImages:[cli1_img, cli2_img, cli3_img],
      description: 'Whitening Cream For Sensitive Areas Moisturizer, Softener, And Soothing Leaves A Feeling Of Freshness Throughout The Day Use It As A Daily Moisturizer, Especially If You Suffer From Skin Rash, Itching, Burning Sensation Due To Sweating, White Secretions, Menstruation Period, Thigh Friction, Hair Removal. Suitable For Women Of All Ages And Skin Types',
      categoryId: 0,
      subCategoryId: 1,
      category: 'Hair Care',
      isNew: true,
      isPopular: false,
      popularTitle: '',
      popularDescription: '',
      createdDate: '2024-01-15',
      isOnSale: false,
      isOnQuickAccess: false
    },
    {
      id: 47,
      name: 'Hair Oil Rosemary',
      mainImage: cli1_img,
      otherImages:[cli1_img, cli2_img, cli3_img],
      description: 'Whitening Cream For Sensitive Areas Moisturizer, Softener, And Soothing Leaves A Feeling Of Freshness Throughout The Day Use It As A Daily Moisturizer, Especially If You Suffer From Skin Rash, Itching, Burning Sensation Due To Sweating, White Secretions, Menstruation Period, Thigh Friction, Hair Removal. Suitable For Women Of All Ages And Skin Types',
      categoryId: 0,
      subCategoryId: 1,
      category: 'Hair Care',
      isNew: true,
      isPopular: false,
      popularTitle: '',
      popularDescription: '',
      createdDate: '2024-01-15',
      isOnSale: false,
      isOnQuickAccess: false
    },
    {
      id: 48,
      name: 'Hair Oil Rosemary',
      mainImage: cli1_img,
      otherImages:[cli1_img, cli2_img, cli3_img],
      description: 'Whitening Cream For Sensitive Areas Moisturizer, Softener, And Soothing Leaves A Feeling Of Freshness Throughout The Day Use It As A Daily Moisturizer, Especially If You Suffer From Skin Rash, Itching, Burning Sensation Due To Sweating, White Secretions, Menstruation Period, Thigh Friction, Hair Removal. Suitable For Women Of All Ages And Skin Types',
      categoryId: 0,
      subCategoryId: 1,
      category: 'Hair Care',
      isNew: true,
      isPopular: false,
      popularTitle: '',
      popularDescription: '',
      createdDate: '2024-01-15',
      isOnSale: false,
      isOnQuickAccess: false
    },
    {
      id: 49,
      name: 'Hair Oil Rosemary',
      mainImage: cli1_img,
      otherImages:[cli1_img, cli2_img, cli3_img],
      description: 'Whitening Cream For Sensitive Areas Moisturizer, Softener, And Soothing Leaves A Feeling Of Freshness Throughout The Day Use It As A Daily Moisturizer, Especially If You Suffer From Skin Rash, Itching, Burning Sensation Due To Sweating, White Secretions, Menstruation Period, Thigh Friction, Hair Removal. Suitable For Women Of All Ages And Skin Types',
      categoryId: 0,
      subCategoryId: 1,
      category: 'Hair Care',
      isNew: true,
      isPopular: false,
      popularTitle: '',
      popularDescription: '',
      createdDate: '2024-01-15',
      isOnSale: false,
      isOnQuickAccess: false
    },
    {
      id: 50,
      name: 'Hair Oil Rosemary',
      mainImage: cli1_img,
      otherImages:[cli1_img, cli2_img, cli3_img],
      description: 'Whitening Cream For Sensitive Areas Moisturizer, Softener, And Soothing Leaves A Feeling Of Freshness Throughout The Day Use It As A Daily Moisturizer, Especially If You Suffer From Skin Rash, Itching, Burning Sensation Due To Sweating, White Secretions, Menstruation Period, Thigh Friction, Hair Removal. Suitable For Women Of All Ages And Skin Types',
      categoryId: 0,
      subCategoryId: 1,
      category: 'Hair Care',
      isNew: true,
      isPopular: false,
      popularTitle: '',
      popularDescription: '',
      createdDate: '2024-01-15',
      isOnSale: false,
      isOnQuickAccess: false
    }
]

export default items_data;